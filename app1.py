...
class MQTTThread(QRunnable):
    ...
    # create a function to flag if the MQTT client is connected
    def on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            self.mqtt_connected = True
            self.on_new_message("Connected to MQTT broker")
        else:
            self.on_new_message("Failed to connect to MQTT broker")
    ...
    def run(self):
        ...
        while not self.client.connected_flag: #wait in loop
            if self.app.stop_flag == True:
                ...
                break
            self.on_new_message("No connection to MQTT broker, check connection")
            time.sleep(1)

        if self.client.connected_flag == True:
            self.client.subscribe(self.topic)
            self.client.on_message = self.on_message
            self.on_new_message("Connected to MQTT broker and waiting for data...")
